...
class MoveItSDK {
    ...
    async joinTransfer(pin) {
        ...
        if (callData.type === 'api-transfer') {
            ...
            const response = await fetch(`https://moveit-api.onrender.com/receive/${pin}`);
...
window.generateApiKey = async () => {
    try {
        const res = await fetch('https://moveit-api.onrender.com/api/v1/keys/generate', {
            method: 'POST',
...
async function fetchUsage(key) {
    try {
        const res = await fetch('https://moveit-api.onrender.com/api/v1/usage', {
...
window.registerPresence = async (mode, pin = null) => {
    try {
        ...
        await fetch('https://moveit-api.onrender.com/api/v1/presence', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
window.checkNearby = async () => {
    try {
        const res = await fetch('https://moveit-api.onrender.com/api/v1/nearby');
